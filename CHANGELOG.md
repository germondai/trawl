...
### Added
- **DataDome support.** Detect the Device Check interstitial, the slider CAPTCHA and the `t=bv` hard block from their `captcha-delivery.com` markers, plus the `x-dd-b` header for header-only proxy escalation. The Device Check runs through a dedicated browser waiter that waits for a fresh `datadome` cookie; the slider is surfaced as `datadome-captcha-required` for a future solver. DataDome escalations run on a small headful sub-pool behind an Xvfb display, because DataDome fails any headless browser while every other supported wall resolves headless. Enable it with `BROWSER_HEADFUL_POOL_SIZE=1`: it is off by default because it adds a browser on top of `BROWSER_POOL_SIZE`, and it is warmed on first use.
